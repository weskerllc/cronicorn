...
export default function TimelineSection({ tabData }: TimelineSectionProps) {
    ...
    return (
        <div className=" bg-muted/35 rounded-md">
            ...
            {/* <div className=" border-border/20 py-5 px-4 hidden lg:block">
                <div className="flex items-start gap-3 text-sm">
                    <span className="text-muted-foreground font-medium text-xs">Automated:</span>
                    <div className="space-y-2.5">
                        {currentBenefits.map((item, index) => (
                            <div key={index} className="flex items-start gap-2">
                                ...
                                <span className="text-muted-foreground leading-relaxed text-xs">{item}</span>
                            </div>
                        ))}
                    </div>
                </div>
